...
public class Hexsphere : MonoBehaviour {
    ...
    public Material[] GroupMaterials_HexExtruded;
    public Material[] GroupMaterials_PentExtruded;

    //The scale multiplier for the entire planet
    [SerializeField]
	public float planetScale = 10f;
    ...
    public void BuildPlanet()
    {
        ...
        Dictionary<Vector3, TileHex> tilesByFaceVerts = new Dictionary<Vector3, TileHex>();

        tiles = new List<TileHex>();
        // Create the tile meshes : O(n)
        foreach (var kvp in trianglesByTileFaceVerts)
        {
            TileHex t = CreateTile(kvp.Key, kvp.Value);
        ...
        {
            List<TileHex> neighbors = new List<TileHex>();
    ...
    public TileHex CreateTile(Vector3 center, List<MeshTriangle> adjacentTris)
    {
        GameObject tileObj = new GameObject("TileHex");
        ...
        TileHex tile = tileObj.AddComponent<TileHex>();
